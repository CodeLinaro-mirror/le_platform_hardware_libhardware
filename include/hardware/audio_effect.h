...
enum effect_command_e {
   ...
   EFFECT_CMD_OFFLOAD_V2,           // Support for tunnel and non tunnel offload effect threads
                                    // send io handle if tunnel effect
                                    // send asm session id, topo id and app type if non tunnel
   EFFECT_CMD_SET_PERSIST,          // set if effect param calibration needs to be persist.
...
// structure used by EFFECT_CMD_OFFLOAD_V2 command
typedef struct effect_offload_v2_param_s {
    bool isOffload;         // true if the playback thread the effect is attached to is offloaded
...
// Effect library interface version 3.1
...
#define EFFECT_LIBRARY_API_VERSION EFFECT_MAKE_API_VERSION(3,1)
